...
Locker::Locker(MDSRank *m, MDCache *c) : need_snapflush_inodes(member_offset(CInode, item_caps)), mds(m), mdcache(c) {}
...
struct MarkEventOnDestruct
{
  ...
  bool mark_event;
  MarkEventOnDestruct(MDRequestRef &_mdr, std::string_view _message) : mdr(_mdr),
                                                                       message(_message),
                                                                       mark_event(true) {}
  ~MarkEventOnDestruct()
  {
    ...
    else if (p.is_wrlock())
    {
      ...
      if (object->is_auth())
      {
        mustpin.insert(object);
               ...
               !mdr->is_peer())
      { // if we are peer (remote_wrlock), the leader already authpinned
...
public:
  C_MDL_DropCache(Locker *l, MDLockCache *lc) : LockerContext(l), lock_cache(lc) {}
    ...
    while (1)
    {
      if (mut->locking && // started xlock (not preempt other request)
          ...
          !(lock->get_state() == LOCK_LOCK_XLOCK && // client is not xlocker or
            in && in->issued_caps_need_gather(lock)))
...
public:
  C_Locker_RevokeStaleCap(Locker *l, CInode *i, client_t c) : LockerContext(l), in(i), client(c)
  {
    in->get(CInode::PIN_PTRWAITER);
  }
  void finish(int r) override
...
public:
  C_MDL_CheckMaxSize(Locker *l, CInode *i, uint64_t _new_max_size,
                     uint64_t _newsize, utime_t _mtime) : LockerContext(l), in(i),
...
public:
  C_Locker_RetryRequestCapRelease(Locker *l, client_t c, const ceph_mds_request_release &it) : LockerContext(l), client(c), item(it) {}
  void finish(int r) override
  {
    string dname;
...
public:
  C_Locker_RetryKickIssueCaps(Locker *l, CInode *i, client_t c, ceph_seq_t s) : LockerContext(l), in(i), client(c), seq(s)
  {
    in->get(CInode::PIN_PTRWAITER);
  }
  void finish(int r) override
                            ...
                            bool *need_flush)
{
  ...
  if (m->flockbl.length())
  {
    ...
    for (int i = 0; i < num_locks; ++i)
    {
      ...
      in->get_fcntl_lock_state()->held_locks.insert(pair<uint64_t, ceph_filelock>(decoded_lock.start, decoded_lock));
    ...
    for (int i = 0; i < num_locks; ++i)
    {
      ceph_filelock decoded_lock;
      decode(decoded_lock, bli);
      in->get_flock_lock_state()->held_locks.insert(pair<uint64_t, ceph_filelock>(decoded_lock.start, decoded_lock));
...
public:
  C_Locker_RetryCapRelease(Locker *l, client_t c, inodeno_t i, uint64_t id,
                           ceph_seq_t mseq, ceph_seq_t seq) : LockerContext(l), client(c), ino(i), cap_id(id), migrate_seq(mseq), issue_seq(seq) {}
  void finish(int r) override
  {
...
void Locker::remove_client_cap(CInode *in, Capability *cap, bool kill)
{
  ...
  if (in->is_auth())
  {
    ...
    if (in->get_projected_inode()->client_ranges.count(client) &&
        !(in->get_inode()->nlink == 0 && !in->is_any_caps()))
...
public:
  C_Locker_ScatterWB(Locker *l, ScatterLock *sl, MutationRef &m) : LockerLogContext(l), lock(sl), mut(m) {}
  void finish(int r) override
  {
...
void Locker::file_eval(ScatterLock *lock, bool *need_issue)
{
  ...
  if (lock->get_state() == LOCK_EXCL)
  {
    ...
    if (!((loner_wanted | loner_issued) & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GBUFFER)) ||
        ...
        (in->is_dir() && in->multiple_nonstale_caps()))
    { // FIXME.. :/
  ...
  // * -> excl?
           ...
           //! lock->is_waiter_for(SimpleLock::WAIT_WR) &&
           in->get_target_loner() >= 0 &&
           (in->is_dir() ? !in->has_subtree_or_exporting_dirfrag() : (wanted & (CEPH_CAP_GEXCL | CEPH_CAP_GWR | CEPH_CAP_GBUFFER))))
  {
...
void Locker::scatter_mix(ScatterLock *lock, bool *need_issue)
{
  ...
  else
  {
    ...
    if (in->is_replicated())
    {
      if (lock->get_state() == LOCK_SYNC_MIX)
